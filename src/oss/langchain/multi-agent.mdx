...
def transfer_to_sales(
    ...
    return Command(
        ...
        update={
            ...
            "messages": [ToolMessage(content="Transferred to sales agent", tool_call_id=tool_call_id)],  # [!code highlight]
        },
  ...
  async (_, config) => {
    return new Command({
      ...
      update: {
        ...
        messages: [new ToolMessage({ content: "Transferred to sales agent", tool_call_id: config.toolCall.id })],  // [!code highlight]
      },
      graph: Command.PARENT
    });
  },
...
class MultiAgentState(AgentState):
    active_agent: str  # [!code highlight]
...
def transfer_to_sales(
    ...
    return Command(
        ...
        update={
            "active_agent": "sales_agent",
            "messages": [transfer_message],  # [!code highlight]
        },
        graph=Command.PARENT
    )
...
def transfer_to_support(
    ...
    return Command(
        goto="support_agent",  # [!code highlight]
        update={
...
def route_to_agent(state: MultiAgentState) -> Literal["sales_agent", "support_agent"]:
    """Route to the active agent based on state, default to sales agent if not set."""
    return state["active_agent"] or "sales_agent"  # [!code highlight]
...
graph.invoke(
    {
        "messages": [{"role": "user", "content": "How much does a flight from Boston to New York cost?"}]
    }
)
```
:::
...
:::js
```typescript
import { StateGraph, START, MessagesAnnotation, Annotation, Command } from "@langchain/langgraph";
...
const MultiAgentState = Annotation.Root({
  ...
  activeAgent: Annotation<string>({  // [!code highlight]
    reducer: (x, y) => y ?? x,
  })
});
...
const transferToSales = tool(
  async (_, config) => {
    ...
    return new Command({
      ...
      update: {
        activeAgent: "sales_agent",
        messages: [transferMessage],  // [!code highlight]
      },
      graph: Command.PARENT
    });
  },
...
const routeToAgent = (state: typeof MultiAgentState.State): string => {
  // Route to the active agent based on state, default to sales agent if not set
  return state.activeAgent || "sales_agent";  // [!code highlight]
};
