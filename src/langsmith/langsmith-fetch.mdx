...
The CLI will automatically fetch traces or threads in `LANGSMITH_PROJECT`. Replace `your-project-name` with the name of your LangSmith project (if it doesn't exist, it will be created automatically on first use). You can also specify a project by its UUID via a CLI flag if needed. `langsmith-fetch` requires only `LANGSMITH_PROJECT` environment variable. It automatically looks up the project UUID and saves both to `~/.langsmith-cli/config.yaml`.

### Use a coding agent with `langsmith-fetch`

After you've installed and set up `langsmith-fetch`, start your favorite coding agent and ask questions like the following:
...
In most cases, you won’t need to find IDs manually (the CLI uses your project name and latest traces by default). However, if you want to fetch a specific item by ID, you can get these IDs from the [LangSmith UI](https://smith.langchain.com):
...
- `pretty` (default): A human-readable view with Rich text formatting (colored panels, message breakdowns, etc.) for easy inspection in terminal. This format is great for quick debugging of a single trace or thread.
...
You can fetch a single thread or trace with the ID. The command will output to the terminal by default; you may optionally redirect it to a file using the `--file` option.
