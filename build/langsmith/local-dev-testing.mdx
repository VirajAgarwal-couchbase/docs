...
<Tip>
For a full reference details, refer to the [LangGraph CLI reference](/langsmith/cli) page.
</Tip>
...
The [`langgraph up`](/langsmith/cli#up) command orchestrates a full Docker-based stack that mirrors production infrastructure, which can help you catch deployment issues before they reach production. The key features include:
...
- **Pre-deployment validation**: Before deploying to production, you can run a final check with a fresh build:
...
Before deploying an application, verify the following with `langgraph dev`:
...
Both `langgraph dev` and `langgraph up` read your application's [dependencies](/langsmith/application-structure#dependencies) from your [configuration files](/langsmith/configuration-file), but they run in different environments:
